...
public static class Guard
{
    ...
    public static void ForMaxLength<TException>(string? value, int maxLength, string name = "Value")
        where TException : BaseDomainException, new()
    {
        if (value?.Length <= maxLength || string.IsNullOrEmpty(value))
        {
            return;
        }

        ThrowException<TException>($"{name} must have maximum {maxLength} symbols.");
    }
